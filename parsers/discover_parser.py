"""
Discover Financial Services Credit Card Statement Parser
"""
...
class DiscoverParser(BaseParser):
    ...
    def parse(self, pdf_path: str, verbose: bool = False) -> Optional[Dict]:
        ...
        try:
            ...
            if verbose:
                print("Extracting data from Discover statement...")
            
            # Extract billing cycle
            billing_cycle = self.extract_billing_cycle_discover(text)
            
            # Extract payment due date
            payment_due_date = self.extract_due_date_discover(text)
            
            # Extract card last 4 digits
            card_last_4 = self.extract_card_last_4_discover(text)
            
            # Extract total balance
            total_balance = self.extract_total_balance_discover(text)
            
            # Extract transactions
            transactions = self.extract_transactions_discover(pdf_path, verbose)
    ...
    def extract_billing_cycle_discover(self, text: str) -> Optional[Dict]:
        """Extract billing cycle from Discover statement"""
        # Discover format: "Statement Period: 01/01/24 - 01/31/24"
        patterns = [
    ...
    def extract_due_date_discover(self, text: str) -> Optional[str]:
        """Extract payment due date from Discover statement"""
        # Discover format: "Payment Due Date: 02/25/24"
        patterns = [
    ...
    def extract_card_last_4_discover(self, text: str) -> Optional[str]:
        """Extract card last 4 digits from Discover statement"""
        # Discover format: "xxxx xxxx xxxx 1234"
        patterns = [
            r'(?:card ending|account ending|card number ending)[:\s]+(\d{4})',
    ...
    def extract_total_balance_discover(self, text: str) -> Optional[float]:
        """Extract total balance from Discover statement"""
        # Discover format: "NEW BALANCE $1,234.56"
        patterns = [
            r'new balance[:\s]+\$?([\d,]+\.?\d*)',
    ...
    def extract_transactions_discover(self, pdf_path: str, verbose: bool = False) -> list:
        """Extract transactions from Discover statement"""
        transactions = []
        try:
